...
That exec line belongs to the RTMP ingress; the direct listeners run their own ffmpeg from the entrypoint's listener block. It is as it stands in `src/nginx-transcoder/nginx-no-ssl.conf`, which is the canonical copy: read the rationale for `-b:a 1024k` and for keeping `$name` on `-i` only in the comment block directly above that line in [`nginx-transcoder/nginx-no-ssl.conf`](https://github.com/mormegil6/Earshot/blob/master/nginx-transcoder/nginx-no-ssl.conf), rather than repeated here. `nginx.conf` (the `SSL_ENABLED=true` variant) must carry the identical exec line, so diff the two whenever either changes: if they drift, an SSL deployment runs an audio configuration nothing has tested.
