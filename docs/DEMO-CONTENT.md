...
The stack always has something to play. The `loop-source` service loops `content/demo.mp4` into the pipeline as if it were a live contribution; when that file is absent, it synthesises a placeholder at first start instead, in-container and without network: a black sphere with a test pattern as a front screen and one 440 Hz source orbiting the listener in 3rd-order Ambisonics, so looking around audibly works before any real content exists. `scripts/make-demo-loop.sh` produces the same placeholder from the host.

`DEMO_CONTENT` in `.env` governs the automation (default `1`). It also covers the VOD reference-master fetch when `VOD_ENABLED=1`. Set `DEMO_CONTENT=0` to do neither; the loop then idles until you provide `demo.mp4`. The file is checked once at `loop-source` startup, so after adding it run `docker compose restart loop-source`.

## What the file must be

H.264 video plus 16-channel AAC with PCE headers, in MP4 (or FLV). The requirement comes from the RTMP contribution leg: legacy RTMP cannot carry VP9 or Opus, and loop-source streams the file with `-c copy` (zero CPU), so nothing re-encodes it on the way in. Channel order must be ACN/SN3D (AmbiX) end to end.
