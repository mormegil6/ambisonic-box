...
<div align="center"><img src="images/obs-windows/13_OBS-recording-settings.png" width="92%" alt="OBS Settings, Output, Recording tab on Windows: Custom Output (FFmpeg), Output to URL, mpegts, 6000 Kbps, keyframe interval 60, libx264, 384 Kbps audio, tracks 1 to 4, aac. The URL in the screenshot is the OLDER guest one, srt://<host>:8890?streamid=<name>&latency=2000000&pkt_size=1128; type the owner URL from the table above instead."></div>

<p align="center"><em><strong>Type the URL from the table above, not the one in this screenshot.</strong> The picture predates the owner route and still shows the keyless guest port <code>:8890</code>, which is off unless you deliberately turn it on - and when it is off, the refusal happens inside the SRT handshake, so OBS can only tell you the connection failed. Every other setting shown is correct, and so are the two URL parameters that matter: <code>&amp;latency=2000000</code>, since SRT's default of about 120 ms survives loopback and little else, and <code>&amp;pkt_size=1128</code>, for the reason in <a href="#5-point-obs-at-the-box">the tunnel trap below</a>.</em></p>
