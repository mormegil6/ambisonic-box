...
Verified on [OBS Studio](https://obsproject.com/) 32.2.1 and [REAPER](https://www.reaper.fm/) 7.78 with ReaRoute, Windows 11. SRT is Secure Reliable Transport, the contribution protocol this stack recommends: stock OBS speaks it, and it carries the four audio tracks that become the 16 channels.
...
Identical on every platform, and authoritative: it reports the value the gateway is running with rather than what a file says it should be. Same name in `.env` and in the container, so there is nothing to translate. Reading the file works too, if the stack is not up: `findstr SRT_OWNER_PASSPHRASE .env` on Windows, `grep SRT_OWNER_PASSPHRASE .env` elsewhere.
...
<p align="center"><em>The URL shown is the owner route on <code>:8891</code>, with the passphrase left as a placeholder; <code>scripts/setup.sh</code> prints yours already filled in, so <strong>paste that rather than retyping this</strong>. It carries both parameters that matter: <code>&amp;latency=2000000</code>, since SRT's default of about 120 ms survives loopback and little else, and <code>&amp;pkt_size=1128</code>, for the reason in <a href="#5-point-obs-at-the-box">the tunnel trap below</a>. <code>127.0.0.1</code> is right when OBS runs on the box itself; otherwise use the address you reach it on.</em></p>

**Do not let a space in front of `srt://` survive the paste.** It is invisible in that field and it does not fail the way you would expect: ffmpeg stops recognising `srt://` as a protocol at all and treats the whole string as a *filename*. On Windows a filename containing `:` and `?` is an illegal name rather than a missing one, so OBS reports **`Invalid argument`** on a URL that looks perfect, and nothing ever reaches the network. On macOS the same paste says `No such file or directory` instead, which reads as "the box is unreachable" and is equally misleading. Both were diagnosed the hard way on 2026-08-10. If you meet either message, click into the field, press Home, and delete one character before believing anything else.
...
| `Couldn't open '...', Invalid argument` on a URL that looks correct | A leading space before `srt://`. ffmpeg then treats the whole string as a filename, and on Windows one containing `:` and `?` is an illegal name. Press Home in the field and delete one character. Check the port too: anything above 65535, such as `88901` for `8891`, gives the same message. |
...
Two Windows-specific notes on running those commands: if both WSL and Git Bash are installed, plain `bash` may resolve to WSL, which mounts Windows drives at `/mnt/c/...` where Git Bash uses `/c/...` - check the shell prompt before assuming a path. And a Windows clone of this repo may check the scripts out with CRLF line endings, which breaks the `#!/usr/bin/env bash` shebang with an opaque `bash\r: No such file or directory`; the repo now ships a `.gitattributes` forcing LF, so a fresh clone is fine. A clone made before that landed is not: git applies the rule at checkout, so `git pull` rewrites only the files whose content also changed, and anything untouched since keeps its CRLF. Run `git add --renormalize .` and then `git checkout -- .` to convert the rest in place (two separate commands: `&&` is not valid in Windows PowerShell).
