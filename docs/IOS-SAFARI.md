...
- **A Safari tab can lose all audio output while every API reports success, and the mechanism is now identified: the tab's audio session collapses to ringer-muted Ambient.** Measured 2026-08-29 through 09-02: `AudioContext.state` stays `running`, an `AnalyserNode` on the output reads normal program RMS (0.02 to 0.08), a plain `<audio>` element plays to completion, fresh contexts behave identically, and nothing is audible; reloads and back/forward do not clear it, a typed address-bar navigation or a Safari quit does. The WebKit source trail explains all of it: the web process's `RemoteAudioSession` caches the audio-session category and dedups `SetCategory` IPCs, and never invalidates that cache when the GPU process (owner of the one real `AVAudioSession`) dies and relaunches; a page that pinned `navigator.audioSession.type = 'playback'` therefore can never re-push its category, the fresh GPU-side proxy defaults to a value iOS maps to `AVAudioSessionCategoryAmbient`, and Ambient is hard-muted by the Ring/Silent switch while every render callback keeps firing. The same GPU relaunch re-runs `load()` on the media element, which is the media-session teardown above, so the teardown and the silence are one trigger. Confirmed on-device 2026-09-02 (session rn60n8): with a wedged, silent tab, flipping the physical Ring/Silent switch to ring restored audible output instantly, flipping back re-muted it, and cycling `navigator.audioSession.type` to `'ambient'` and back to `'playback'` restored output while still on silent, which a plain re-assert of `'playback'` cannot do because the dedup guard swallows an unchanged value. The player now performs that cycle on every `pageshow` and inside the in-place recovery, so a wedged session heals itself; on a healthy stack the cycle is two redundant IPCs. Filed as [WebKit bug 323104](https://bugs.webkit.org/show_bug.cgi?id=323104); the source-level chain and the workaround are reported there. The earlier reading of this entry, that the fault sits below every API and cannot be worked around from the page, is superseded by the cycle above.
...
The player decodes and renders third-order Ambisonics on iOS, on-demand and live, verified on an iPhone Xs (A12, iOS 18.7) and on macOS Safari 27. Two failures remain open. The first is a media-session teardown, now traced to a MediaSource detach: `removeAttribute('src')` plus `load()` closes a ManagedMediaSource permanently, and dash.js's `ScheduleController` latches off on the resulting `endstreaming` and never re-reads the source, so it is also the cause of the zero-request failure. That mechanism was reproduced on macOS Safari 27 with stock dash.js; the field trigger on iOS is a silent GPU-process relaunch, the same event behind the audio fault below, and it is reported as [dash.js issue 5127](https://github.com/Dash-Industry-Forum/dash.js/issues/5127) with the recovery opened as [PR 5128](https://github.com/Dash-Industry-Forum/dash.js/pull/5128). The second was a WebKit audio-output fault, now traced end to end: a GPU-process relaunch leaves the web process's cached audio-session category stale, the tab collapses to ringer-muted Ambient, and every API keeps reporting success; confirmed on-device by the Ring/Silent flip and healed by cycling `navigator.audioSession.type`, which the player now does automatically, filed with the full chain as [bug 323104](https://bugs.webkit.org/show_bug.cgi?id=323104). The live pipeline also still emits no keep-alive track. That track remains wanted for the separate, measured backgrounding behaviour described above, but it is not the fix for the teardown.
