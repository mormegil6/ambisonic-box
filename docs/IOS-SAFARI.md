...
- The live path still emits no keep-alive track, so backgrounding behaviour is proven on static fixtures only. The track is wanted for that behaviour and is not a fix for the teardown above. See [Keeping the audio alive when the window is not visible](#keeping-the-audio-alive-when-the-window-is-not-visible).
