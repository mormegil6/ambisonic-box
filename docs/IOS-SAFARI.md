...
- The `videojs-contrib-dash` gate that blocked iPhone is cleared as of player rf31: its dist registered a DASH source handler only when `window.MediaSource` existed, and inlined a dash.js 4.2.0 that predates `ManagedMediaSource`, the only media engine iOS ships. The fork now bundles the package from source, so video rides the same patched dash.js 4.7.4 as the rest of the player and the handler registers on MMS-only WebKit too.
- Clearing that gate exposed a second one, in dash.js itself, and it is the one that actually mattered for iPhone. 4.7.4 gates every segment request, the init segment included, on a flag that ManagedMediaSource `endstreaming` sets false and only `startstreaming` clears. Real Safari sends `endstreaming` for a paused element holding no data, so the player deadlocks: WebKit will not ask for data for a stream that cannot play, and dash.js will not fetch until asked. Measured in real Safari 27 on 2026-08-28 as zero segment requests with `readyState` pinned at 0. Player rf33 honours the hint only once a quality is initialised and the buffer is non-empty, which is what `endstreaming` means; `?forcemms=1` forces a dual-engine browser onto the MMS branch so the iPhone path can be exercised where a debugger exists. Real Safari on that forced path then played 101 segments with no stall over 75 s.
- Note what this says about test coverage: Playwright's WebKit reproduces NEITHER failure. Both were found by driving real Safari through `safaridriver`. Treat headless WebKit as evidence about WebKit's parser and codecs, not about its media session behaviour.
- A run of the real player on a physical iPhone is still owed. Everything above is desktop Safari on the same engine, which is the closest available proxy and not the thing itself.
