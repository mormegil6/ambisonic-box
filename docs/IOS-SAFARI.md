...
- **The media session tears down under a decode error dash.js answers silently.** Traced to dash.js's own shipped source on 2026-08-31, correcting an earlier reading here that the element's error code was 4 (`SRC_NOT_SUPPORTED`): `StreamController` intercepts `MEDIA_ERR_DECODE` (code 3) specifically, and within a configurable retry budget (`errors.recoverAttempts.mediaErrorDecode`, default 5) detaches and re-attaches the `ManagedMediaSource` without ever firing a public event. `element.removeAttribute('src'); element.load()` is the resulting call, and it is the exact `abort`/`emptied` signature this player's own instrumentation captured six independent times on 2026-08-31, every one within one millisecond of a `MANAGED_MEDIA_SOURCE_END_STREAMING` event. Code 4 would always surface publicly under dash.js's own logic, which does not fit a session that needs polling to detect; code 3's silent path does. If the re-attach never receives a `sourceopen` back, the session hangs indefinitely with nothing observable on the page at all; when it does, the retry is invisible from outside and playback simply continues. What raises the underlying decode error is not established. Two specific candidates were tested directly on 2026-08-31 and ruled out as required triggers: a stale same-origin instance left over from `pagehide`'s bfcache handling competing for a `ManagedMediaSource` grant, and the recovery watchdog being unable to check a backgrounded tab. The clean signature reproduced on a freshly restarted, single-tab, actively foregrounded session with neither condition present. This player's own watchdog detects the torn-down session and reloads within about 30 seconds, but only on a tab that gets any CPU time to run the check at all; a fully backgrounded tab was measured sitting dead for over 10 minutes straight with no recovery, since the browser itself never scheduled a single check. The defect that starts the teardown is dash.js's and is not fixed here.
- **The Safari content process can lose all audio output while every API reports success.** Measured directly on 2026-08-29: `AudioContext.state` stays `running`, its clock keeps advancing, an `AnalyserNode` on the output confirms real samples, a plain `<audio>` element plays a short clip to completion with no error, and a brand-new `AudioContext` created afterward is equally silent, yet nothing is audible. The state survived closing the original context and reloading the page, and cleared only when Safari itself was quit. No signal reaches the page that distinguishes this from a healthy session, so it cannot be detected or worked around here; see "What still costs something" below. It is not specific to the live player: on 2026-08-31 it also reproduced on the on-demand clip page, which shares no manifest, no packaging pipeline and no live-manifest handling with the live path, confirming the fault tracks the browser process rather than any one page's code.
...
- Audio on iPhone, live or on-demand, can go silent mid-session with no observable cause and no player-side recovery, because the fault sits below every API the page can query; force-quitting Safari is the only known fix. See [the note above the requirements](#live-failure-note).
- A media session can tear down under a `MEDIA_ERR_DECODE` element error; the player recovers it within about 30 seconds on a foregrounded tab, but a fully backgrounded one can stay dead far longer since the browser withholds the CPU time the recovery check needs, and neither case prevents the teardown itself. See [the note above the requirements](#live-failure-note).
...
The player decodes and renders third-order Ambisonics on iOS, on-demand and live, verified on an iPhone Xs (A12, iOS 18.7) and on macOS Safari 27. Two intermittent failures remain open, mainly on the live stream: a media-session teardown, now traced to dash.js's own silent decode-error retry and partially mitigated (the player's watchdog recovers a foregrounded tab within about 30 seconds, but not a fully backgrounded one), and a WebKit audio-output fault outside the player's reach, reproduced on both the live and on-demand pages, that only a Safari restart clears. Neither is fixable from this codebase; both are written up for their respective upstream projects. The live pipeline also still emits no keep-alive track, so the backgrounding behaviour described above is proven on static fixtures only.
