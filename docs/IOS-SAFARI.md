...
The disagreement runs the other way for 16-channel Opus in WebM, and it is the more dangerous direction. On an iPhone Xs running iOS 18.7 with Safari 18.7.7, measured 2026-09-12 with the four-surfaces reproduction page, the two string surfaces both say yes: `canPlayType` answers "probably" and `ManagedMediaSource.isTypeSupported` answers true (plain `MediaSource` does not exist there). Both decoders then refuse the same file: `decodeAudioData` invokes its error callback with null, and WebCodecs reports `isConfigSupported` false. Stereo Opus is accepted by all four surfaces on the same device, so the refusal is the channel count, not the codec or the container. A player that trusts either string surface before attempting a decode will select a track it cannot play. On that same run a WebAssembly libopus decoded all sixteen channels, mapping family 255, in 104 ms, with every channel of the tone ladder detected at its expected frequency, so the ACN order survives the decode, and the listener heard it.
