...
One exception, disclosed rather than glossed over: the musical bed loop under `directions` (`directions-bed_16ch.wav`) is of undetermined origin. It was sourced some time ago from what was believed to be a freely licensed stock source that can no longer be traced or verified. It ships under the same CC BY 4.0 terms in good faith; if a rights holder identifies it and objects, it will be removed or replaced on request.
