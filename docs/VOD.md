...
One exception, which the CC BY 4.0 grant above does **not** cover: the musical bed loop under `directions` (`directions-bed_16ch.wav`) is [Earshot](https://github.com/EnvelopSound/Earshot)'s test asset `tester/resources/16chambixloop.wav`, byte for byte. Earshot is GPL-2.0, and where the loop came from before that is not recorded there, so it is redistributed under Earshot's terms rather than ours. If a rights holder identifies it and objects, it will be removed or replaced on request.
