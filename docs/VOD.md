...
One exception, and the CC BY 4.0 grant above does **not** extend to it: the musical bed loop under `directions` (`directions-bed_16ch.wav`). It is byte-for-byte [Earshot](https://github.com/EnvelopSound/Earshot)'s own test asset, `tester/resources/16chambixloop.wav` (identical file size, sample count and decoded-PCM checksum), which Earshot's README offers as a ready-made 16-channel source for testing. Earshot is licensed GPL-2.0.

Two things follow, and both are stated plainly rather than resolved, because only Envelop can resolve them. Earshot's repository does not record where the loop itself came from, so while its proximate source is now certain, the ultimate rights holder is not: Envelop may have authored it, licensed it, or used stock material of their own. And GPL-2.0 is a copyleft software licence that does not grant the terms offered above, so this one component is redistributed here under Earshot's licence rather than under CC BY 4.0, and anyone reusing `directions` should treat the bed accordingly.

An earlier version of this paragraph described the loop as being of undetermined origin and untraceable. That was written in good faith and is simply no longer true; it was traced in August 2026. If a rights holder identifies it and objects, it will be removed or replaced on request.
