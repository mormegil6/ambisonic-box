...
- **the two SRT gateways are drawn as one box.** There are really two instances of the same image: `srt-gateway` (guest, 8890/udp, in the base compose) and `srt-gateway-owner` (8891/udp, written into the override by `scripts/setup.sh`). They differ only in mode, and drawing both would double the busiest corner of the picture, so one box carries both labels. What the box does NOT flatten is the port: the two inbound SRT edges are labelled `:8891, owner` and `:8890, guest`, because which port a sender dials is the whole difference between the two routes, and that is the one thing a sender has to get right. The outgoing edges are real for the same reason: the guest route republishes over RTMP into `rtmp-ingest`, and the owner route (default `SRT_DIRECT=1`) hands MPEG-TS straight to earshot, skipping ingest and the 16-channel AAC re-encode it would otherwise force.
- **owner routes are drawn above guest routes, everywhere.** Both parallel pairs (SRT into the gateway, RTMP into ingest) put owner on top. That is not decoration: the owner routes are on by default, while every guest route exists only where the operator switched it on. Ordering comes straight from edge order in `architecture.mmd` - the first edge of a repeated pair takes the top slot - so swapping two lines in the source swaps them in the drawing.
- **`*` on a label means "off by default".** It marks the two guest edges, and the legend under the box carries the condition (`GUEST_ENABLED=1`). One character rather than a word because the labels have no room for a clause; the legend is where the clause lives. Note this is a different statement from the dotted edges, which mean "not part of the live path at all" (shaka's offline packaging, telemetry's freshness probe).
...
- **contribution is a 2x2, and only one of the four cells is now folded away.** Both roles can arrive over both transports: an OWNER pushes SRT to `:8891` (the owner gateway) or RTMP to `:1935/owner` (key-authed), and a GUEST pushes SRT to `:8890` (the guest gateway) or RTMP to `:1935/guest` (keyless, arbiter-gated) - stock OBS over SRT or OBS Music Edition over RTMP, per `docs/GUEST-ENDPOINT.md`. The drawing now carries three of those four explicitly: both SRT ports on the gateway's input side, and both applications on its output side. The one it still flattens is OBS Music Edition's single `RTMP :1935` edge, because that sender genuinely can address either application from one port - the role is chosen by the path it types, not by a second port to draw.
