...
**STATUS 2026-09-01: FIXED AND VERIFIED, M153 BACKPORT UNCERTAIN.** 547065816 is now Verified (not just Fixed), confirmed on Canary 154.0.8021.0 on 2026-08-24. A child merge request for M153, [555381064](https://issues.chromium.org/issues/555381064), was opened 2026-08-26 but as of 2026-09-01 is still Assigned and explicitly marked **Safe Merge: No**, which under Chromium's own merge policy routes it to Release Manager Review rather than an automatic cherry-pick; no reviewer decision is recorded yet. Read that as genuinely uncertain, not as "in progress and arriving soon": a request can sit in that state, get rejected, or land, and this page has no way to predict which. The one confirmed date remains **M154, stable 2026-09-22**. Re-check both tracker links before telling anyone to upgrade, and retire the `--disable-features=DirectOpusAudioDecoding` workaround only once a shipped stable is measured clean.
