...
if [ -e "$ENV_FILE" ]; then
    ...
    if [ -n "${owner_key:-}" ] && [ -n "${owner_pass:-}" ]; then
        ...
        # every platform - caught live by the first Windows install of v1.0.0. Only written into a FRESH .env: an
        # existing deployment that builds from source must never be switched
        # to pulling by a setup re-run.
        ...
        say "  (runs from the published $PIN_TAG images; README covers building from source)"
    else
...
services:
  srt-gateway-owner:
    ...
    # Building from source instead? Use image: ambi-box-srt-gateway:local
    # and add:   build: { context: ./services/srt-gateway }
