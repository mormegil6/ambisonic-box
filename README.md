...
A **guest** stream has its four tracks combined into one 16-channel AAC stream and is handed on over RTMP to the same ingest the RTMP route uses. That extra step is deliberate: it is where guests are authorised, counted, time-limited and, if need be, cut off, so every one of those protections applies to SRT senders unchanged.
