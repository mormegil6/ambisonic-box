...
| `srt-gateway` | SRT contribution ingest (native OBS multitrack); joins 4x4 to 16-ch, republishes into the guest arbiter. Privilege-separated; on by default, admits nobody unless `GUEST_ENABLED=1` | 8890/udp |
...
- **Resource guard (`GUEST_MAX_TEMP_C`, `GUEST_MAX_MBPS`, both off by default):** nothing else stops a guest pushing 60 Mbps at your hardware. Temperature is the one to trust, because it is measured on the device and therefore sees room heat, dust and thermal soak; bitrate is a coarse pre-filter for the obvious abuser. The distinction is not theoretical: on the 2012 Mac Mini deployment a synthetic 42 Mbps ladder peaked at 87 C while a real 45.5 Mbps session ran 87-96 C with zero viewers, a ~9 C gap no bitrate threshold could have predicted. That same ladder (4K30, video passed through) measured 8 Mbps -> 72 C peak, 18 -> 76, 42 -> 87, 59 -> 93, with the encoder never dropping below realtime - a thermal ceiling, not a capability one. Set both from a measurement of YOUR host rather than copying these numbers; neither limit fires until it holds for `GUEST_LIMIT_STRIKES` consecutive pings, and one good sample forgives the strikes.
