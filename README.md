...
(`/vod/?clip=<name>`). Two ship with the repo: `directions` (spoken
front/right/left/top orientation test over an ambisonic energy-visualisation
overlay) and `colortones` (colour-and-tone A/V-sync pattern, fully generated by
`scripts/make-colortones.sh`). Masters live in `content/vod/masters/`;
