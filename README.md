...
> **REACHABLE on the public demo at [stream.bmroz.eu](https://stream.bmroz.eu/) since 2026-08-20.** The inbound port landed: the department forwards UDP 8890 to the box, and a guest session has been carried end to end from a machine outside the faculty network - admitted by the arbiter, taking the direct-to-DASH path, sixteen channels of third-order ambisonics at 17.4 Mbps, with the demo loop standing down for the handover and the reconnect grace arming on disconnect. Two caveats worth stating in the same breath rather than further down. The forwarding rules are **provisional** until the administrator makes them permanent, so the public endpoint can disappear on a restart of equipment nobody here controls. And the forward **rewrites source addresses**: every external caller reaches the box as one address, which makes the per-IP ban described below a blunter instrument than it reads - see the note beside it. If you are running your own instance none of this ever applied: open the port on your own host and the endpoint is reachable immediately.
>
> **What has and has not been proven, stated plainly.** An automated suite exercises admission, the single slot, the kick lever and sanitising of the one string a guest controls on **every push**. The session cap, cooldown, the reconnect grace, bans and fail-closed behaviour with the arbiter down are in the same suite, but it runs **nightly**: it takes about 17 minutes, and a gate that slow gets ignored rather than read. A real guest session has been carried end to end from a **third-party machine** over plain IP - admitted, taking the direct-to-DASH path, with all sixteen channels arriving in the right order at the right levels - and guest handover measures 0.1 s on amd64 and 1.0 s worst case on arm64 against a 4 s budget. The arm64 figure is a handover measurement on a Raspberry Pi 4, not a run of the whole suite on that board.
> What has NOT happened: **no unknown person has ever used this endpoint.** The other half of this sentence expired on 2026-08-20 - it HAS now been exposed to the public internet, and the first session across that path is described above - but the half that matters is still true, so it stays. Every session to date, that one included, was pushed from a machine and a network we control. An open port is not the same thing as surviving strangers, and this line stays written here until someone we did not brief pushes to it.
